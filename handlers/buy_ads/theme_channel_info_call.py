...
theme_channel_info_callback = CallbackData('channel_info', 'id', 'page', "sort_by", 'reverse', "theme")
...
async def channel_info_call(call: CallbackQuery, data: CallbackData):
    ...
    btns=(InlineKeyboardButton(text=time,callback_data='buy_s_time$' + id + '$' +time) for time, price in channel_conditions)
    kb.add(*btns)
    kb.row(InlineKeyboardButton("Назад", callback_data=theme_channels_callback.new('BACK',data['page'], data['sort_by'], data['reverse'], theme)))
    
    kb.insert(InlineKeyboardButton(text=' ___ ', callback_data='незнаю'))
    
    try:
        kb.insert(InlineKeyboardButton(text='NEXT', callback_data=theme_channels_callback.new(str(id)+'$nt',page, sort_by, reverse, theme)))
    except ValueError: print(str(id)+'$nt',page, sort_by, reverse, theme)
